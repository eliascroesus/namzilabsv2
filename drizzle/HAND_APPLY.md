...
## 0034 — `workspace_templates` + `workspace_template_uses` + `dashboard_notes` (sharing a board as a template)

Three tables, nothing altered. `workspace_templates` holds each shared template:
a FROZEN snapshot of some views' structure — positions, chart kinds, text
blocks, group names, notes and app names, never a metric, a number or a
connection — plus the random `code` its public link `/t/<code>` carries.
`workspace_template_uses` counts who used which (the recipient's workspace is its
`org_id`). `dashboard_notes` is where a groups view's column, or a calendar view,
keeps the "which metric goes here" note that a custom view's tile keeps in its
own `config`.

**Safe to paste before or after the deploy — and the deploy is built to survive
it arriving late.** Nothing that already exists reads these tables: the board
treats a missing `dashboard_notes` as "no notes", workspace deletion skips the
three if they are absent, and every template screen says "Templates aren't
switched on yet" instead of throwing. What does NOT work until this is pasted:
creating a template, opening a template link, and writing a note on a column.
...
CREATE INDEX IF NOT EXISTS "workspace_templates_org_idx" ON "workspace_templates" USING btree ("org_id");
...
CREATE INDEX IF NOT EXISTS "workspace_template_uses_template_idx" ON "workspace_template_uses" USING btree ("template_id");
CREATE INDEX IF NOT EXISTS "workspace_template_uses_org_idx" ON "workspace_template_uses" USING btree ("org_id");
...
```sql
SELECT count(*) AS should_be_3 FROM information_schema.tables
WHERE table_schema='public' AND table_name IN ('workspace_templates','workspace_template_uses','dashboard_notes');
```

Then make one: Settings → Templates → New template, open its link in a private
window, and

```sql
SELECT code, name, version, enabled FROM workspace_templates ORDER BY created_at DESC LIMIT 5;
```
...
CREATE UNIQUE INDEX IF NOT EXISTS "access_grants_referrer_rung_uq" ON "access_grants" USING btree ("referrer_user_id","referral_rung") WHERE referral_rung is not null;
