...
describe("Sendblue messages poll + webhook health", () => {
  ...
  /**
   * "0 loaded" against an account with 853 sent messages. The HTTP call
   * succeeded, so nothing errored — the connector simply read a key that was not
   * there and `?? []` turned "I did not understand this response" into "there is
   * nothing here". Two independent places did it, one per layer, and the two are
   * indistinguishable to the user. Both are now either parsed or reported.
   */
  describe("never answers a misread response with zero", () => {
    it("finds the message list inside Sendblue's documented {status, message, data} envelope", async () => {
      vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({ status: "OK", message: "Success", data: { messages: [msg("h1", 5)] } })));
      const { records } = await sendblueConnector.poll!({ connectionId: "c1", cursor: null, credentials: { apiKey: "a", apiSecret: "b" } });
      expect(records.map((r) => r.eventId)).toEqual(["sendblue:c1:sms_delivered:h1"]);
    });

    it.each([
      ["bare array", (m: unknown) => [m]],
      ["{messages}", (m: unknown) => ({ messages: [m] })],
      ["{data: []}", (m: unknown) => ({ status: "OK", data: [m] })],
      ["{results}", (m: unknown) => ({ results: [m] })],
      ["{data: {results}}", (m: unknown) => ({ status: "OK", data: { results: [m] } })],
    ])("reads the list when it arrives as %s", async (_label, wrap) => {
      vi.stubGlobal("fetch", vi.fn(async () => jsonResponse(wrap(msg("h1", 5)))));
      const { records } = await sendblueConnector.poll!({ connectionId: "c1", cursor: null, credentials: { apiKey: "a", apiSecret: "b" } });
      expect(records).toHaveLength(1);
    });

    it("errors — naming the keys that did arrive — when the list is nowhere it knows", async () => {
      vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({ status: "OK", payload: { records: [msg("h1", 5)] } })));
      const err = (await sendblueConnector
        .poll!({ connectionId: "c1", cursor: null, credentials: { apiKey: "a", apiSecret: "b" } })
        .catch((e) => e as Error)) as Error;
      expect(err).toBeInstanceOf(Error);
      expect(err.message).toContain("status, payload"); // what actually came back
      expect(err.message).toContain("message list");
    });

    it("errors when messages arrive but the date field has been renamed", async () => {
      // The second silent zero: parsed fine, then the first-sync floor rejected
      // every message because none had a date we could read.
      vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({
        messages: [{ message_handle: "h1", status: "DELIVERED", is_outbound: true, delivered_on: T(5) }],
      })));
      const err = (await sendblueConnector
        .poll!({ connectionId: "c1", cursor: null, credentials: { apiKey: "a", apiSecret: "b" } })
        .catch((e) => e as Error)) as Error;
      expect(err.message).toContain("readable date");
      expect(err.message).toContain("delivered_on"); // the key we did get
    });

    it("still reports a genuinely empty account as zero, not as an error", async () => {
      vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({ status: "OK", data: { messages: [] } })));
      const { records } = await sendblueConnector.poll!({ connectionId: "c1", cursor: null, credentials: { apiKey: "a", apiSecret: "b" } });
      expect(records).toEqual([]);
    });

    it("an old-but-real account is zero records, not an error (the floor did its job)", async () => {
      // Dates parse fine; they are simply older than the 30-day first-sync floor.
      vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({ messages: [msg("h1", -60 * 24 * 60)] })));
      const { records } = await sendblueConnector.poll!({ connectionId: "c1", cursor: null, credentials: { apiKey: "a", apiSecret: "b" } });
      expect(records).toEqual([]);
    });
  });
