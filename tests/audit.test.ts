...
describe("the audit trail records the acts that matter", () => {
  it("covers every governance act, by name", () => {
    ...
    for (const required of [
      ...
      // A workspace's STRUCTURE leaving it, by public link.
      "template.create",
      "template.update",
  ...
  it("uses no action outside the closed list in src/lib/audit.ts", () => {
    ...
    // plus billing.referral_reward and the eight admin.* acts (26 Sep 2026).
    expect(declared.size, "the AuditAction union could not be parsed — this check would pass vacuously").toBe(34);
