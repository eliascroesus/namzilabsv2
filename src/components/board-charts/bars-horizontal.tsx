    ...
    <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto quiet-scroll">
      {shown.map((g) => (
        <div key={g.label} className="flex items-center gap-2" data-tip={`${g.label} · ${formatMetricValue(g.value, format)}`}>
          <span className="min-w-0 max-w-24 flex-1 truncate text-xs text-muted-foreground" title={g.label}>
            {g.label}
          </span>
          ...
          <span className="tnum max-w-24 shrink-0 truncate text-right text-xs text-foreground" title={formatMetricValue(g.value, format)}>
