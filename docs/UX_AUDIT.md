...
### 10d. One row, and the motion layer

**The canvas controls came up onto the chrome row.** Three islands on one line
at a 24px inset — identity left, canvas controls centre, state and actions
right — with the Island component, its radius and its padding untouched. Only
where they live changed.

Two consequences had to be handled, and measuring found both:

- **The left island is the only one that shrinks.** With a plain `min-w-0` the
  flex row crushed it to **16px** — its own padding, back button and all, with
  the contents spilling out of a box that had ceased to exist. The canvas
  controls and the actions are fixed clusters (`shrink-0`); the breadcrumb
  floors at 130px, which is the chevron plus a truncating crumb. Measured
  across canvas widths: 276px at 1292, 140px at 1050, 130px at 900 — where the
  row genuinely no longer fits, which is inherent to putting everything on one
  line and is stated rather than hidden.
- **The panel took the foot back.** With nothing at the bottom to clear, it is
  `top-chrome-band bottom-6`. The band is a TOP measurement now, and its pin
  matters more rather than less: `top-chrome-band` is the token's *only*
  reference, so deleting that one class name removes the variable from the
  build entirely.

**And the kit was lying in the other direction.** The chrome specimen rendered
at the page's 816px column, where the row overflows and the right island falls
off the box — something the product does not do at any width a laptop has. It
now renders on a 1292px canvas (a 1440 viewport minus the rail) inside a
sideways-scrolling box: the truth, at the width the chrome actually has.

### 10e. Motion, in three rules

Not many animations — one curve, three durations, and a strict rule about what
is allowed to move.

- **`--ease-soft`, `--dur-fast/base/slow`**, declared in plain `:root` and *not*
  in `@theme`, because Tailwind emits a theme variable only when a generated
  utility references it and these are consumed by hand-written CSS. In `@theme`
  they would silently not exist — the trap `--radius-frame` already fell into.
  Every existing animation was retuned onto them, so nothing invents its own
  timing any more.
- **The canvas glides.** Layout here is managed: every insert, delete and branch
  re-runs the layout and moves every card. Without a transition they teleport —
  you press "+" and the flow blinks into a different arrangement. Transitioning
  the node's transform makes the glide *the explanation*: it shows you the steps
  moved down rather than that the screen changed. Safe here specifically
  because nothing drags — with `nodesDraggable` off, the only thing that ever
  changes a node's inline translate is a re-layout.
- **A card arrives, a panel slides, a modal pops.** The arrival animation is on
  the CARD, never on React Flow's node wrapper — an animation there would
  override the inline translate and slam every node to the origin mid-flight.
  The docked panel slides 16px from the edge it is attached to instead of
  scaling from its own centre, which read as a dialog that happened to be over
  there; `flow-pop-in` stays with the centred modals. All of it uses
  `backwards`, never `both`, so no transform survives the animation to
  re-anchor the fixed-position field browser.

Plus a `prefers-reduced-motion: reduce` backstop for anything that slips
through as an inline `transition-*` utility. State still changes instantly and
visibly; it just does not travel.
