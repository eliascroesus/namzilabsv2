...
| **Google Sheets** | Google OAuth (already have Drive+Sheets scope on namzilabs.co) | **No native "new row" webhook.** Options: Drive API `files.watch` push, or an Apps Script `onEdit/onChange` POST | n/a (Drive channel token) | **Polling with a row cursor is the reliable primary.** | Drive push: HTTPS + valid SSL required, **domain must be verified in Google Cloud**, channels **expire (≤7 days for file changes; default 1 h if unset), no auto-renew — must re-`watch`**, notifications only ~every 3 min (batched) and **don't say what changed** → you still read the sheet. Sheets read quota: **300/min per project, 60/min per user** (read off this project's Cloud console; Drive is 12,000/min, Calendar 10,000/min per project and 600/min per user — declared in `catalog.ts`). Drive being 40x Sheets is why the two are separate budget buckets, and why the polling probe is effectively free: **Drive push is PARKED** rather than blocked — domain verification is a human prerequisite that buys almost nothing at that quota. [Drive push](https://developers.google.com/workspace/drive/api/guides/push) |
